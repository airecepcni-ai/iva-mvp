  ...
  for (const file of files) {
    try {
      ...
      if (statResult.isDirectory()) {
        ...
        if (path.resolve(filePath) === path.join(apiDir, 'route.js')) {
...
function getHonoPath(routeFile: string): { name: string; pattern: string }[] {
  const routeFilePosix = toPosix(routeFile);
  const relativePath = routeFilePosix.replace(apiDirPosix, '');
          ...
          if (route[method]) {
            ...
            const handler: Handler = async (c) => {
              ...
              if (import.meta.env.DEV) {
                const updatedUrl = pathToFileURL(routeFile).href;
                const updatedRoute = await import(
                  /* @vite-ignore */ `${updatedUrl}?update=${Date.now()}`
                );
...
// Export for lazy initialization - routes should be registered after neonConfig is set up
export { api, API_BASENAME, registerRoutes };
